...
-map <F2> :cn<CR>
 ...
 map <F12> :set spelllang=sv<CR>
        ...
        """.strip())
        self.assertEquals(2, len(diff.files[0].hunks))
        self.assertEquals(
            '@@ -6,7 +6,6 @@ let g:snippets_dir="~/.vim/snippets"',
            diff.files[0].hunks[0].line.content)
        self.assertEquals(
            '@@ -80,7 +79,6 @@ endfunction',
            diff.files[0].hunks[1].line.content)

    def test_parses_diff_lines_in_hunks(self):
        diff = self.parser.parse("""
diff --git a/.vimrc b/.vimrc
...
+set shiftwidth=4
 ...
 set noignorecase
        ...
        """.strip())
        parts = diff.files[0].hunks[0].parts
